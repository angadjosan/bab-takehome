...
contract Deploy is Script {
    ...
    function run() external {
        ...
        if (keccak256(bytes(set)) == keccak256("mainnet")) p = MarketParams.mainnet();
        else if (keccak256(bytes(set)) == keccak256("demo")) p = MarketParams.demo();
        else revert("PARAM_SET must be demo or mainnet");
