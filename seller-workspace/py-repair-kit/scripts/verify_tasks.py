...
def protocol_checks(srv: Server, tmp: Path) -> None:
    print("-- protocol checks (T1)")
    wd = str(tmp / "ep-protocol")
    srv.call(cmd="reset", taskId="T1", seed=0, workdir=wd)
    bad = [
        ...
        ({"type": "write_file", "path": "ledgerlite/conftest.py", "content": ""}, "not_editable"),
    ...
    while True:
        ...
        if r["actionsRemaining"] == 0:
            break
    check(r["actionsUsed"] == 12 and not r["done"], f"protocol: budget accounting wrong {r}")
    r = srv.call(cmd="step", action={"type": "read_file", "path": "ledgerlite/lru.py"})
...
def main() -> int:
    tmp = Path(tempfile.mkdtemp(prefix="pyrepair-verify-"))
    srv = Server()
    rows = []
    try:
