...
def to_cents(value: Number) -> Decimal:
    ...
    to ``0.00``.
    """
    q = _to_decimal(value).quantize(CENT, rounding=ROUND_HALF_EVEN)
...
def allocate(total: Number, weights: Sequence[Union[int, Decimal]]) -> list[Decimal]:
    ...
    cents_total = int(to_cents(total) * 100)
    sign = -1 if cents_total < 0 else 1
    cents = abs(cents_total)
    exact = [cents * f / wsum for f in fracs]
    shares = [math.floor(e) for e in exact]
    leftover = cents - sum(shares)
    order = sorted(range(len(exact)), key=lambda i: (-(exact[i] - shares[i]), i))
    for i in order[:leftover]:
        shares[i] += 1
    return [(Decimal(sign * s) / 100).quantize(CENT) for s in shares]
